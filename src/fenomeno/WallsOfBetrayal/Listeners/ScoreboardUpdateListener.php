...
class ScoreboardUpdateListener implements Listener
{
    ...
    public function onLoyaltyChange(LoyaltyChangeEvent $event): void
    {
        ...
        $loyaltyPercentage = $loyaltyManager->getLoyaltyPercentage($player);
        $loyaltyRank = $loyaltyManager->getLoyaltyRank($player);
        
        $this->updateScoreboard($player, 6, "§6Loyalty: §f{$loyaltyPercentage}% " . ($loyaltyRank?->getColor() ?? "§7") . ($loyaltyRank?->value ?? ""));
    }
    ...
    private function setScoreboard(Player $player): void
    {
        ...
        $lines = [
            ...
            "§r§7",
            "§6Loyalty: §f{$loyaltyPercentage}% " . ($loyaltyRank?->getColor() ?? "§7") . ($loyaltyRank?->value ?? ""),
            "§7Score   : §f1290 pts",
            "§7",
            "§cplay.qoia.com"
        ];
