...
class RemovePermissionCommand extends WCommand
{
    ...
    public function onRun(CommandSender $sender, string $aliasUsed, array $args): void
    {
        ...
        Await::f2c(function () use ($sender, $permission, $playerName) {
            try {
                yield from $this->main->getRolesManager()->removePermission($playerName, $permission);

                MessagesUtils::sendTo($sender, MessagesIds::PERMISSION_PLAYER_REMOVED, [
                    ExtraTags::PLAYER     => $playerName,
                    ExtraTags::PERMISSION => $permission
                ]);
            } catch (PlayerDontHasPermissionException) {
                MessagesUtils::sendTo($sender, MessagesIds::PLAYER_DONT_HAVE_PERMISSION, [
                    ExtraTags::PLAYER => $playerName,
                    ExtraTags::PERMISSION => $permission
                ]);
    ...
    {
        return CommandsConfig::getCommandById(CommandsIds::REMOVE_PERMISSION);
    }
}