...
class AddSubRoleCommand extends WCommand
{
    ...
    public function onRun(CommandSender $sender, string $aliasUsed, array $args): void
    {
        ...
        Await::f2c(function () use ($sender, $role, $playerName) {
            try {
                yield from $this->main->getRolesManager()->addSubRole($playerName, $role);

                MessagesUtils::sendTo($sender, MessagesIds::SUBROLE_PLAYER_SET, [
                ...
                if ($player) {
                    MessagesUtils::sendTo($player, MessagesIds::SUBROLE_SET, [
                        ExtraTags::SUBROLE => $role->getDisplayName(),
                    ]);
                }
            } catch (PlayerAlreadyHasSubRoleException) {
                MessagesUtils::sendTo($sender, MessagesIds::ALREADY_HAS_SUBROLE, [
                    ExtraTags::PLAYER => $playerName,
    ...
    {
        return CommandsConfig::getCommandById(CommandsIds::ADD_SUB_ROLE);
    }
}